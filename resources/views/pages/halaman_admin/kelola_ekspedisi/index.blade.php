...
@section('content')
    <!-- Start Content ekspedisi -->


    <main class="h-full pb-16 overflow-y-auto">
        <div class="container grid px-6 mx-auto">
            <h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">
                Kelola Data ekspedisi
            </h2>
            ...
            <table id="example" class="display border border-slate-200" style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th class="border border-slate-200  text-center">Alamat</th>
                        <th class="border border-slate-200  text-center">Nomor HP</th>
                ...
                <tbody>
                    ...
                    @foreach ($ekspedisi as $baris)
                        <tr>
                            <td class="border border-slate-200 text-center">{{ $loop->iteration }}</td>
                            <td class="border border-slate-200 text-center">{{ $baris->nama_ekspedisi }}</td>
                            <td class="border border-slate-200 whitespace-pre-line text-center">
                                {{ $baris->alamat_ekspedisi }}
                            </td>

                            <td class="border border-slate-200 whitespace-pre-line text-center">
                                {{ $baris->no_hp_ekspedisi }}
                            </td>
                                ...
                                <div class="flex items-center justify-center text-sm">
                                    ...
                                    <div class="relative hidden w-32 h-32 rounded-full md:block">
                                        <img class="object-cover w-full h-full rounded-lg"
                                            src="{{ asset($baris->gambar_ekspedisi) }}" alt="" loading="lazy" />
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="border border-slate-200">
                                <div class="flex items-center space-x-4 text-sm">
                                    <a href="{{ route('ekspedisi.edit', $baris->id_ekspedisi) }}">
                                        <button
                                    ...
                                    <form action="{{ route('ekspedisi.destroy', $baris->id_ekspedisi) }}" method="post"
